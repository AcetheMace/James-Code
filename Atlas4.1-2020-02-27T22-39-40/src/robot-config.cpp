...
motor right_arm = motor(PORT11, ratio18_1, false);
motor left_arm = motor(PORT12, ratio18_1, false);
motor arm_lift = motor(PORT1, ratio6_1, true);
...
int rc_auto_loop_callback_Controller1() {
  ...
  while(true) {
    if(RemoteControlCodeEnabled) {
      ...
      // check the Up/Down Buttons status to control arm_lift
      if (Controller1.ButtonUp.pressing()) {
        arm_lift.spin(forward);
        Controller1UpDownButtonsControlMotorsStopped = false;
      } else if (Controller1.ButtonDown.pressing()) {
        arm_lift.spin(reverse);
        Controller1UpDownButtonsControlMotorsStopped = false;
      ...
      // check the X/B buttons status to control lift
